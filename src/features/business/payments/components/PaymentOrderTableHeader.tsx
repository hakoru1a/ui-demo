...
interface PaymentOrderTableHeaderProps {
  table: Table<PaymentOrder>;
  // CSV Export
  csvData: PaymentOrder[];
  ...
  // Actions
  onBulkDelete?: (selectedPaymentOrders: PaymentOrder[]) => void;
  onExportExcel?: () => void;
  onExportPDF?: () => void;
}
...
function StatusTabs({ table, statusFilter, onStatusFilterChange }: StatusTabsProps) {
  ...
  const statusCounts = useMemo(() => {
    ...
    return {
      ...
      paid: allRows.filter((row) => row.original.status === 'paid').length
    };
  }, [table]);
  ...
  const getTabColor = (value: StatusFilter | string) => {
    ...
    if (value === 'paid') return theme.palette.success.main;
    if (value === 'draft') return theme.palette.grey[600];
  ...
  const tabsConfig = useMemo(
    () => [
      ...
      {
        value: 'paid' as StatusFilter | string,
        label: STATUS_OPTIONS.find((opt) => opt.value === 'paid')?.label || 'Đã thanh toán',
...
function FilterPopover({ open, onClose, anchorEl, columnFilters, onFilterChange }: FilterPopoverProps) {
  ...
  // Date range state for payment date
  const paymentDateRange = getDateRangeValue('paymentDate') || {};
  const [paymentStartDate, setPaymentStartDate] = useState<Date | null>(paymentDateRange.start ? new Date(paymentDateRange.start) : null);
  const [paymentEndDate, setPaymentEndDate] = useState<Date | null>(paymentDateRange.end ? new Date(paymentDateRange.end) : null);

  const handlePaymentStartDateChange = (value: Date | null) => {
    setPaymentStartDate(value);
    const dateStr = value ? dateHelper.formatDate(value, 'YYYY-MM-DD') : undefined;
    handleDateRangeChange('paymentDate', dateStr, paymentDateRange.end);
  };

  const handlePaymentEndDateChange = (value: Date | null) => {
    setPaymentEndDate(value);
    const dateStr = value ? dateHelper.formatDate(value, 'YYYY-MM-DD') : undefined;
    handleDateRangeChange('paymentDate', paymentDateRange.start, dateStr);
  };
                  ...
                  <Grid container spacing={2}>
                    ...
                    <Grid size={{ xs: 12, sm: 4 }}>
                      <SelectField
                        ...
                        options={[{ value: '', label: 'Tất cả' }, ...PAYMENT_ORDER_TYPE_OPTIONS]}
                        fullWidth
                        size="medium"
                      />
                    </Grid>
                    ...
                    <Grid size={{ xs: 12, sm: 4 }}>
                      <SelectField
                        label="Đối tác"
                        value={getFilterValue('partnerType') || ''}
                        onChange={(e) => handleFilterChange('partnerType', e.target.value)}
                        options={[{ value: '', label: 'Tất cả' }, ...PARTNER_TYPE_OPTIONS]}
                        fullWidth
                        size="medium"
                      />
                    </Grid>

                    {/* Ngày thanh toán - Date Range */}
                      ...
                      <Box>
                        <Grid container spacing={1}>
                          <Grid size={{ xs: 12, sm: 6 }}>
                            <DatePickerField
                              label="Từ ngày"
                              value={paymentStartDate ? dateHelper.normalizeDateValue(paymentStartDate) : null}
                              onChange={(newValue) => handlePaymentStartDateChange(newValue ? newValue.toDate() : null)}
                              format="DD/MM/YYYY"
                              slotProps={{
                                textField: {
                                  fullWidth: true,
                          ...
                          <Grid size={{ xs: 12, sm: 6 }}>
                            <DatePickerField
                              label="Đến ngày"
                              value={paymentEndDate ? dateHelper.normalizeDateValue(paymentEndDate) : null}
                              onChange={(newValue) => handlePaymentEndDateChange(newValue ? newValue.toDate() : null)}
                              format="DD/MM/YYYY"
                              slotProps={{
                                textField: {
                                  fullWidth: true,
                    ...
                    <Grid size={{ xs: 12, sm: 3 }}>
                      <SelectField
                        ...
                        options={[{ value: '', label: 'Tất cả' }, ...PAYMENT_METHOD_OPTIONS]}
                        fullWidth
                        size="medium"
                      />
                    </Grid>
...
const PaymentOrderTableHeader = ({
  table,
  csvData,
  csvHeadersData,
  csvFilename = 'payment-orders',
  columnFilters,
  onFilterChange,
  ...
  // Only draft payment orders can be bulk deleted
  const canBulkDelete = hasSelection && selectedRows.every((row) => row.original.status === 'draft');

  const handleCreateNew = () => {
    navigate(PAYMENT_ORDER_URLS.NEW);
  };

  const handleBulkDelete = () => {
    if (onBulkDelete && canBulkDelete) {
      const selectedPaymentOrders = selectedRows.map((row) => row.original);
      onBulkDelete(selectedPaymentOrders);
    }
  };
    ...
    <>
      ...
      >
        <Stack direction="row" spacing={2} alignItems="center" sx={{ width: '100%' }}>
          <TextField
            placeholder="Tìm kiếm theo mã PO, đối tác..."
            value={searchValue}
...
export default PaymentOrderTableHeader;
