...
interface PaymentOrderTableProps {
  table: TanStackTable<PaymentOrder>;
  data: PaymentOrder[];
  loading?: boolean;
...
const PaymentOrderTable = ({ table, data, loading = false, initialPageSize = 10 }: PaymentOrderTableProps) => {
  // Render table
  const renderTable = () => {
    if (loading) {
    ...
    if (data.length === 0) {
      return <EmptyTable msg="No payment orders found" />;
    }
