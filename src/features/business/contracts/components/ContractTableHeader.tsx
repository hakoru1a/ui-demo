...
import { CONTRACT_TYPE_OPTIONS, CONTRACT_URLS, PARTNER_TYPE_OPTIONS, STATUS_OPTIONS } from '../types/constants';
...
interface ContractTableHeaderProps {
  ...
  // Actions
  onBulkDelete?: (selectedContracts: Contract[]) => void;
  onExportExcel?: () => void;
  onExportPDF?: () => void;
}
...
function StatusTabs({ table, statusFilter, onStatusFilterChange }: StatusTabsProps) {
  ...
  const getTabColor = (value: StatusFilter | string) => {
    ...
    if (value === StatusFilter.ACTIVE || value === 'active') return theme.palette.success.main;
    if (value === 'draft') return theme.palette.grey[600];
    if (value === 'expired') return theme.palette.warning.main;
  ...
  const tabsConfig = useMemo(
    () => [
      ...
      },
      {
        ...
        label: STATUS_OPTIONS.find((opt) => opt.value === 'active')?.label || 'Hiệu lực',
        count: statusCounts.active
      },
      {
        value: 'expired' as StatusFilter | string,
        label: STATUS_OPTIONS.find((opt) => opt.value === 'expired')?.label || 'Hết hạn',
...
function FilterPopover({ open, onClose, anchorEl, columnFilters, onFilterChange }: FilterPopoverProps) {
  ...
  // Date range state for effective date
  const effectiveDateRange = getDateRangeValue('effectiveDate') || {};
  const [effectiveStartDate, setEffectiveStartDate] = useState<Date | null>(
    effectiveDateRange.start ? new Date(effectiveDateRange.start) : null
  );
  const [effectiveEndDate, setEffectiveEndDate] = useState<Date | null>(effectiveDateRange.end ? new Date(effectiveDateRange.end) : null);

  const handleEffectiveStartDateChange = (value: Date | null) => {
    setEffectiveStartDate(value);
    const dateStr = value ? dateHelper.formatDate(value, 'YYYY-MM-DD') : undefined;
    handleDateRangeChange('effectiveDate', dateStr, effectiveDateRange.end);
  };

  const handleEffectiveEndDateChange = (value: Date | null) => {
    setEffectiveEndDate(value);
    const dateStr = value ? dateHelper.formatDate(value, 'YYYY-MM-DD') : undefined;
    handleDateRangeChange('effectiveDate', effectiveDateRange.start, dateStr);
  };
                      ...
                      <Box>
                        <Grid container spacing={1}>
                          <Grid size={{ xs: 12, sm: 6 }}>
                            <DatePickerField
                              label="Từ ngày"
                              value={effectiveStartDate ? dateHelper.normalizeDateValue(effectiveStartDate) : null}
                              onChange={(newValue) => handleEffectiveStartDateChange(newValue ? newValue.toDate() : null)}
                              format="DD/MM/YYYY"
                              slotProps={{
                                textField: {
                                  fullWidth: true,
                          ...
                          <Grid size={{ xs: 12, sm: 6 }}>
                            <DatePickerField
                              label="Đến ngày"
                              value={effectiveEndDate ? dateHelper.normalizeDateValue(effectiveEndDate) : null}
                              onChange={(newValue) => handleEffectiveEndDateChange(newValue ? newValue.toDate() : null)}
                              format="DD/MM/YYYY"
                              slotProps={{
                                textField: {
                                  fullWidth: true,
