...
import type { Stocktake } from '../types/index';

interface StocktakeTableProps {
  table: TanStackTable<Stocktake>;
  data: Stocktake[];
  loading?: boolean;
...
const StocktakeTable = ({ table, data, loading = false, initialPageSize = 10 }: StocktakeTableProps) => {
  // Render table
  const renderTable = () => {
    if (loading) {
    ...
    if (data.length === 0) {
      return <EmptyTable msg="Không tìm thấy phiếu kiểm kê nào" />;
    }
