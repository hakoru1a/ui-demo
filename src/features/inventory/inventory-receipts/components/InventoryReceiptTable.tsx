...
interface InventoryReceiptTableProps {
  table: TanStackTable<InventoryReceipt>;
...
const InventoryReceiptTable = ({ table, data, loading = false, initialPageSize = 10 }: InventoryReceiptTableProps) => {
  // Render table
  const renderTable = () => {
    if (loading) {
    ...
    if (data.length === 0) {
      return <EmptyTable msg="Không tìm thấy phiếu nhập kho nào" />;
    }
