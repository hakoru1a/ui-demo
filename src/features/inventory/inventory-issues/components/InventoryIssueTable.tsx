...
interface InventoryIssueTableProps {
  table: TanStackTable<InventoryIssue>;
  data: InventoryIssue[];
  loading?: boolean;
...
const InventoryIssueTable = ({ table, data, loading = false, initialPageSize = 10 }: InventoryIssueTableProps) => {
  // Render table
  const renderTable = () => {
    if (loading) {
    ...
    if (data.length === 0) {
      return <EmptyTable msg="Không tìm thấy phiếu xuất nào" />;
    }
