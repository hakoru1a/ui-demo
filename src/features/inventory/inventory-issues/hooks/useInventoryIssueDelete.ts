...
// ==============================|| INVENTORY ISSUE DELETE HOOK ||============================== //

/**
 * Hook to manage delete action for inventory issues
 * Handles dialog state, confirmation, and API call with notifications
 */
export function useInventoryIssueDelete(onSuccess?: (deletedIds: string[]) => void) {
  ...
  const handleConfirmDelete = useCallback(async () => {
    ...
    try {
      ...
      if (isBulkDelete) {
        ...
        response = await inventoryIssueService.deleteInventoryIssue(ids[0]);
      }
