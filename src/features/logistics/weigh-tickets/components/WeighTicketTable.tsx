...
interface WeighTicketTableProps {
  table: TanStackTable<WeighTicket>;
  data: WeighTicket[];
  loading?: boolean;
...
const WeighTicketTable = ({ table, data, loading = false, initialPageSize = 10 }: WeighTicketTableProps) => {
  // Render table
  const renderTable = () => {
    if (loading) {
    ...
    if (data.length === 0) {
      return <EmptyTable msg="Không tìm thấy phiếu cân nào" />;
    }
...
export default WeighTicketTable;
