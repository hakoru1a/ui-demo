...
interface PriceTableTableProps {
  table: TanStackTable<PriceTable>;
  data: PriceTable[];
  loading?: boolean;
...
const PriceTableTable = ({ table, data, loading = false, initialPageSize = 10 }: PriceTableTableProps) => {
  // Render table
  const renderTable = () => {
    if (loading) {
    ...
    if (data.length === 0) {
      return <EmptyTable msg="Không tìm thấy bảng giá nào" />;
    }
