...
export type DispatchOrderStatusFilter = 'all' | 'new' | 'running' | 'completed';

interface DispatchOrderTableHeaderProps {
  table: Table<DispatchOrder>;
  // CSV Export
  csvData: DispatchOrder[];
  ...
  onStatusFilterChange?: (status: DispatchOrderStatusFilter) => void;
  // Search
  searchValue?: string;
  onSearchChange?: (value: string) => void;
  // Delete
  selectedRows?: DispatchOrder[];
  onBulkDelete?: (orders: DispatchOrder[]) => void;
  // Feature flags
...
function StatusTabs({ table, statusFilter, onStatusFilterChange }: StatusTabsProps) {
  ...
  const tabsConfig = useMemo(
    () => [
      { value: 'all' as DispatchOrderStatusFilter, label: 'Tất cả', count: statusCounts.all },
      {
        value: 'new' as DispatchOrderStatusFilter,
        label: DISPATCH_ORDER_STATUS_OPTIONS.find((opt) => opt.value === 'new')?.label || 'Mới',
        count: statusCounts.new
      },
      {
        value: 'running' as DispatchOrderStatusFilter,
        label: DISPATCH_ORDER_STATUS_OPTIONS.find((opt) => opt.value === 'running')?.label || 'Đang chạy',
        count: statusCounts.running
      },
      {
        value: 'completed' as DispatchOrderStatusFilter,
        label: DISPATCH_ORDER_STATUS_OPTIONS.find((opt) => opt.value === 'completed')?.label || 'Hoàn thành',
  ...
  const getTabColor = (value: DispatchOrderStatusFilter) => {
    switch (value) {
      case 'all':
        return theme.palette.primary.main;
      case 'new':
        return theme.palette.grey[600]; // Grey for "new" status
      case 'running':
  ...
  return (
    <Box sx={{ borderBottom: 1, borderColor: 'divider', px: 2, pt: 2 }}>
      <Tabs
        value={statusFilter}
        onChange={(_, value) => onStatusFilterChange?.(value as DispatchOrderStatusFilter)}
...
function FilterPopover({ open, onClose, anchorEl, columnFilters, onFilterChange }: FilterPopoverProps) {
  ...
  // Get vehicle options
  const vehicleOptions = getMockVehicles().map((v) => ({
    value: v.id,
                ...
                <Box sx={{ p: 2.5 }}>
                  <Grid container spacing={2}>
                    ...
                    <Grid size={{ xs: 12, sm: 4 }}>
                      <TextField
                        label="Mã lệnh"
                        value={getFilterValue('orderCode') || ''}
                    ...
                    <Grid size={{ xs: 12, sm: 4 }}>
                      <SelectField
                        label="Xe / Tài xế"
                        value={getFilterValue('vehicleId') || ''}
                        onChange={(e) => handleFilterChange('vehicleId', e.target.value)}
                        options={[{ value: '', label: 'Tất cả' }, ...vehicleOptions]}
                        fullWidth
                        size="medium"
                      />
                    </Grid>
                    ...
                    <Grid size={{ xs: 12, sm: 4 }}>
                      <SelectField
                        ...
                        options={[{ value: '', label: 'Tất cả' }, ...DISPATCH_ORDER_STATUS_OPTIONS]}
                        fullWidth
                        size="medium"
                      />
                    </Grid>

                    {/* Thời gian vận chuyển - Date Range */}
                    <Grid size={{ xs: 12, sm: 6 }}>
                      <Stack direction="row" spacing={1} alignItems="center">
                        <DatePickerField
                          label="Từ ngày"
                          value={dateHelper.normalizeDateValue(getFilterValue('dateFrom') as string)}
                          onChange={(newValue) => {
                            handleFilterChange('dateFrom', newValue ? newValue.toISOString() : undefined);
                          }}
                          slotProps={{
                            textField: {
                              fullWidth: true,
                        ...
                        <DatePickerField
                          label="Đến ngày"
                          value={dateHelper.normalizeDateValue(getFilterValue('dateTo') as string)}
                          onChange={(newValue) => {
                            handleFilterChange('dateTo', newValue ? newValue.toISOString() : undefined);
                          }}
                          slotProps={{
                            textField: {
                              fullWidth: true,
...
const DispatchOrderTableHeader = ({
  table,
  csvData,
  csvHeadersData,
    ...
    <>
      ...
      >
        <Stack direction="row" spacing={2} alignItems="center" sx={{ width: '100%' }}>
          <TextField
            placeholder="Tìm kiếm theo mã lệnh, biển số xe..."
            value={searchValue}
          ...
          {hasSelectedRows && onBulkDelete && (
            <Tooltip title={`Xóa ${selectedRows.length} lệnh điều động đã chọn`}>
              <IconButton
                size="medium"
                color="error"
...
export default DispatchOrderTableHeader;
