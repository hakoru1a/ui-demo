...
interface DispatchOrderTableProps {
  table: TanStackTable<DispatchOrder>;
  data: DispatchOrder[];
  loading?: boolean;
...
const DispatchOrderTable = ({ table, data, loading = false, initialPageSize = 10 }: DispatchOrderTableProps) => {
  // Render table
  const renderTable = () => {
    if (loading) {
    ...
    if (data.length === 0) {
      return <EmptyTable msg="Không tìm thấy lệnh điều động nào" />;
    }
