...
interface WorkforceDispatchOrderTableProps {
  table: TanStackTable<WorkforceDispatchOrder>;
  data: WorkforceDispatchOrder[];
  loading?: boolean;
  initialPageSize?: number;
}

// ==============================|| WORKFORCE DISPATCH ORDER TABLE ||============================== //

const WorkforceDispatchOrderTable = ({ table, data, loading = false, initialPageSize = 10 }: WorkforceDispatchOrderTableProps) => {
  // Render table
  const renderTable = () => {
    if (loading) {
    ...
    if (data.length === 0) {
      return <EmptyTable msg="Không tìm thấy lệnh điều phối nào" />;
    }
...
export default WorkforceDispatchOrderTable;
