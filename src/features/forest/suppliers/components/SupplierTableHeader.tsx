...
import { SUPPLIER_URLS, SUPPLIER_TYPE_OPTIONS, CERTIFICATE_OPTIONS, REGION_OPTIONS } from '../types/constants';
...
interface SupplierTableHeaderProps {
  ...
  // Actions
  onBulkDelete?: (selectedSuppliers: Supplier[]) => void;
  onStopCooperation?: (selectedSuppliers: Supplier[]) => void;
}
  ...
  const tabsConfig = useMemo(
    () => [
      ...
      },
      {
        ...
        label: STATUS_OPTIONS.find((opt) => opt.value === 'inactive')?.label || 'Ngưng hợp tác',
...
const SupplierTableHeader = ({
  ...
  const hasActiveSelected = selectedRows.some((row) => row.original.status === 'active');

  const handleCreateNew = () => {
    navigate(SUPPLIER_URLS.NEW);
  };
  ...
  const handleStopCooperation = () => {
    if (onStopCooperation && hasActiveSelected) {
      const activeSuppliers = selectedRows.filter((row) => row.original.status === 'active').map((row) => row.original);
      onStopCooperation(activeSuppliers);
    }
  };
    ...
    <>
      ...
      >
        <Stack direction="row" spacing={2} alignItems="center" sx={{ width: '100%' }}>
          ...
          {/* Ngưng hợp tác - Secondary, Conditional (when status = active) */}
          {hasActiveSelected && onStopCooperation && (
            <Button variant="outlined" color="warning" startIcon={<DeleteOutlined />} onClick={handleStopCooperation}>
              Ngưng hợp tác ({selectedRows.filter((row) => row.original.status === 'active').length})
            </Button>
          )}

          {/* Xóa nhiều NCC - Secondary, Conditional (when ≥1 selected) */}
          {hasSelection && onBulkDelete && (
            <Button variant="outlined" color="error" startIcon={<DeleteOutlined />} onClick={handleBulkDelete}>
              Xóa nhiều NCC ({selectedCount})
            </Button>
          )}

          {/* Tạo nhà cung cấp - Primary, Always */}
