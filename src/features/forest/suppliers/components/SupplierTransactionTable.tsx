...
interface SupplierTransactionTableProps {
  table: TanStackTable<SupplierTransaction>;
...
const SupplierTransactionTable = ({ table, data, loading = false, initialPageSize = 10 }: SupplierTransactionTableProps) => {
  // Render table
  const renderTable = () => {
    if (loading) {
    ...
    if (data.length === 0) {
      return <EmptyTable msg="No transactions found" />;
    }
