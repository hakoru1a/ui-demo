...
// types
import { HarvestPlan } from '../types';

interface HarvestPlanTableProps {
  table: TanStackTable<HarvestPlan>;
  data: HarvestPlan[];
  loading?: boolean;
...
const HarvestPlanTable = ({ table, data, loading = false, initialPageSize = 10 }: HarvestPlanTableProps) => {
  // Render table
  const renderTable = () => {
    if (loading) {
    ...
    if (data.length === 0) {
      return <EmptyTable msg="No harvest plans found" />;
    }
...
export default HarvestPlanTable;
