...
// types
import { HarvestOrder } from '../types';

interface HarvestOrderTableProps {
  table: TanStackTable<HarvestOrder>;
  data: HarvestOrder[];
  loading?: boolean;
...
const HarvestOrderTable = ({ table, data, loading = false, initialPageSize = 10 }: HarvestOrderTableProps) => {
  // Render table
  const renderTable = () => {
    if (loading) {
    ...
    if (data.length === 0) {
      return <EmptyTable msg="Không tìm thấy lệnh khai thác nào" />;
    }
...
export default HarvestOrderTable;
