...
interface VesselTrackingTableProps {
  table: TanStackTable<VesselTracking>;
  data: VesselTracking[];
  loading?: boolean;
...
const VesselTrackingTable = ({ table, data, loading = false, initialPageSize = 10 }: VesselTrackingTableProps) => {
  // Render table
  const renderTable = () => {
    if (loading) {
    ...
    if (data.length === 0) {
      return <EmptyTable msg="Không tìm thấy chuyến tàu nào" />;
    }
...
export default VesselTrackingTable;
