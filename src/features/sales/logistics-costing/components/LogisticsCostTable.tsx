...
interface LogisticsCostTableProps {
  table: TanStackTable<LogisticsCost>;
  data: LogisticsCost[];
  loading?: boolean;
...
const LogisticsCostTable = ({ table, data, loading = false, initialPageSize = 10 }: LogisticsCostTableProps) => {
  // Render table
  const renderTable = () => {
    if (loading) {
    ...
    if (data.length === 0) {
      return <EmptyTable msg="Không tìm thấy chi phí logistics nào" />;
    }
...
export default LogisticsCostTable;
