...
interface ExportOrderTableProps {
  table: TanStackTable<ExportOrder>;
  data: ExportOrder[];
  loading?: boolean;
...
const ExportOrderTable = ({ table, data, loading = false, initialPageSize = 10 }: ExportOrderTableProps) => {
  // Render table
  const renderTable = () => {
    if (loading) {
    ...
    if (data.length === 0) {
      return <EmptyTable msg="Không tìm thấy đơn hàng xuất khẩu nào" />;
    }
