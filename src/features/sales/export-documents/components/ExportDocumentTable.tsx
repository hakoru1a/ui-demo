...
interface ExportDocumentTableProps {
  table: TanStackTable<ExportDocument>;
  data: ExportDocument[];
  loading?: boolean;
...
const ExportDocumentTable = ({ table, data, loading = false, initialPageSize = 10 }: ExportDocumentTableProps) => {
  // Render table
  const renderTable = () => {
    if (loading) {
    ...
    if (data.length === 0) {
      return <EmptyTable msg="Không tìm thấy chứng từ nào" />;
    }
...
export default ExportDocumentTable;
