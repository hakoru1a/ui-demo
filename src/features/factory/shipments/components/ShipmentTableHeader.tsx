...
import { SHIPMENT_URLS, SHIPMENT_TYPE_OPTIONS, SHIPMENT_STATUS_OPTIONS, WAREHOUSE_OPTIONS, CUSTOMER_OPTIONS } from '../types/constants';
...
interface ShipmentTableHeaderProps {
  ...
  onStatusFilterChange?: (status: ShipmentStatusFilter) => void;
  // Search
  searchValue?: string;
  ...
  // Bulk delete
  onBulkDelete?: (shipments: Shipment[]) => void;
...
function StatusTabs({ table, statusFilter, onStatusFilterChange }: StatusTabsProps) {
  ...
  const getTabColor = (value: ShipmentStatusFilter) => {
    switch (value) {
      case ShipmentStatusFilter.ALL:
        return theme.palette.primary.main;
      case ShipmentStatusFilter.DRAFT:
        return theme.palette.warning.main;
      case ShipmentStatusFilter.ISSUED:
        return theme.palette.success.main;
      case ShipmentStatusFilter.CANCELLED:
  ...
  const tabsConfig = useMemo(
    () => [
      { value: ShipmentStatusFilter.ALL, label: 'Tất cả', count: statusCounts.all },
      {
        value: ShipmentStatusFilter.DRAFT,
        label: SHIPMENT_STATUS_OPTIONS.find((opt) => opt.value === 'draft')?.label || 'Nháp',
        count: statusCounts.draft
      },
      {
        value: ShipmentStatusFilter.ISSUED,
        label: SHIPMENT_STATUS_OPTIONS.find((opt) => opt.value === 'issued')?.label || 'Đã xuất',
        count: statusCounts.issued
      },
      {
        value: ShipmentStatusFilter.CANCELLED,
        label: SHIPMENT_STATUS_OPTIONS.find((opt) => opt.value === 'cancelled')?.label || 'Hủy',
                  ...
                  <Grid container spacing={2}>
                    ...
                    <Grid size={{ xs: 12, sm: 4 }}>
                      <SelectField
                        ...
                        options={[{ value: '', label: 'Tất cả' }, ...SHIPMENT_TYPE_OPTIONS]}
                        fullWidth
                        size="medium"
                      />
                    </Grid>
                    ...
                    <Grid size={{ xs: 12, sm: 4 }}>
                      <SelectField
                        ...
                        options={[{ value: '', label: 'Tất cả' }, ...SHIPMENT_STATUS_OPTIONS]}
                        fullWidth
                        size="medium"
                      />
                    </Grid>
