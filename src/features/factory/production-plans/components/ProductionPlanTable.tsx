...
interface ProductionPlanTableProps {
  table: TanStackTable<ProductionPlan>;
  data: ProductionPlan[];
  loading?: boolean;
...
const ProductionPlanTable = ({ table, data, loading = false, initialPageSize = 10 }: ProductionPlanTableProps) => {
  // Render table
  const renderTable = () => {
    if (loading) {
    ...
    if (data.length === 0) {
      return <EmptyTable msg="Không tìm thấy kế hoạch/lệnh sản xuất nào" />;
    }
